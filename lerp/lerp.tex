...
\begin{align*}
  ...
  \right)
  %% \frac{\partial K}{\partial d_n}
  %% &= w_{n-1}^th_{n-1}^v\frac{\partial J_{n-1}}{\partial d_n} \\
  %% &= w_{n-1}^th_{n-1}^v4\left(-\frac{3h_{n-1}}{w_{n-1}^2}+\frac{1}{w_{n-1}}d_{n-1}+\frac{2}{w_{n-1}}d_n\right) \\
  %% &= 4\left(-3w_{n-1}^{t-2}h_{n-1}^{v+1}+w_{n-1}^{t-1}h_{n-1}^vd_{n-1}+2w_{n-1}^{t-1}h_{n-1}^vd_n\right) \\
  %% \frac{\partial K}{\partial d_i}
  %% &= w_{i-1}^th_{i-1}^v\frac{\partial J_{i-1}}{\partial d_i}
  %% + w_i^th_i^v\frac{\partial J_i}{\partial d_i} \\
  %% &= w_{i-1}^th_{i-1}^v4\left(-\frac{3h_{i-1}}{w_{i-1}^2}+\frac{1}{w_{i-1}}d_{i-1}+\frac{2}{w_{i-1}}d_i\right)
  %% + w_i^th_i^v4\left(-\frac{3h_i}{w_i^2}+\frac{2}{w_i}d_i+\frac{1}{w_i}d_{i+1}\right) \\
  %% &= 4\left(
  %% -3w_{i-1}^{t-2}h_{i-1}^{v+1}-3w_i^{t-2}h_i^{v+1}
  %% +w_{i-1}^{t-1}h_{i-1}^vd_{i-1}
  %% +\left(2w_{i-1}^{t-1}h_{i-1}^v+2w_i^{t-1}h_i^v\right)d_i
  %% \right)
\end{align*}
\begin{align*}
  4\left(-\frac{3w_1^{t-2}}{h_1^{v-1}}+\frac{2w_1^{t-1}}{h_1^v}d_1+\frac{w_1^{t-1}}{h_1^v}d_2\right)
