...
Yet another method to generate the Penrose tiling,
discovered by de Bruijn,
is as the dual of a ``pentagrid,''
or 5 symmetrically-arranged sets of parallel lines.
The intersections of these lines represent rhombi
whose edges are parallel to the intersecting lines,
and two adjacent intersections corresponding
to Penrose rhombs that share an edge.
As with the other methods,
these sets of parallel lines may be shifted in the plane
to obtain different tilings,
but unlike with the matching rules,
these are often locally distinguishable.
