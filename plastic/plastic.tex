...
may be considered as an element
of a 3D vector space over the rationals.
Addition is performed elementwise,
as is multiplication by a rational number.
Multiplying two of these numbers together, though,
does not correspond to any of the normal vector space operations,
as it would be a product of two vectors that yields another vector.
However, multiplying by an element of the plastic field
is a linear transformation of the 3D vector space,
so it may be represented by a transformation matrix.
\begin{align*}
